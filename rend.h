...
public:
	...
	void VertexSorting(GzCoord** v1, GzCoord** v2, GzCoord** v3, GzCoord** n1, GzCoord** n2, GzCoord** n3);
	void CalculateColor(const GzCoord& n, GzColor* c);

	// HW3
	int GzDefaultCamera();
	int GzPutCamera(GzCamera camera);
	int GzPushMatrix(const GzMatrix& matrix, bool pushToXnorm = true);
	int GzPopMatrix();
